...
const buildClientRedirectUrl = (params = {}) => {
	const baseUrl = process.env.CLIENT_URL || "http://localhost:5173/login";
...
async function HandleGithubOAuth(req, res) {
	...
	try {
		...
		if (!email) {
			email = `github-${githubId}@github.local`;
		}
		...
		return res.redirect(buildClientRedirectUrl({ oauth: "success", provider: "github" }));
	} catch (err) {
